...
class MultiWozReader(_ReaderBase):
    def __init__(self):
        ...
        if cfg.multi_acts_training or cfg.multi_act_fusion:
            self.multi_acts = json.loads(open(cfg.multi_acts_path, 'r').read())
            if cfg.multi_acts_path2:
                self.multi_acts2 = json.loads(open(cfg.multi_acts_path2, 'r').read())
    ...
    def _get_encoded_data(self, fn, dial):
        ...
        for idx, t in enumerate(dial['log']):
            ...
            if cfg.multi_act_fusion:
                ...
                if fn in self.multi_acts:
                    turn_ma = self.multi_acts[fn].get(str(idx), {})
                    for act_spans in turn_ma:
                        enc['aspn_aug_cat'] += [self.vocab.sentence_encode(act_spans.split())]
                if self.multi_acts2:
                    enc['aspn_aug_cat2'] = []
                    if fn in self.multi_acts2:
                        turn_ma = self.multi_acts2[fn].get(str(idx), {})
                        for act_spans in turn_ma:
                            enc['aspn_aug_cat2'] += [self.vocab.sentence_encode(act_spans.split())]
    ...
    def convert_batch(self, py_batch, py_prev, first_turn=False):
        ...
        if cfg.multi_act_fusion:
            py_list = []
            batch_aspn_aug = py_batch['aspn_aug_cat']
            if cfg.rs and cfg.mode == 'train':
                p = np.random.rand()
                if p > cfg.rs_p:
                    batch_aspn_aug = py_batch['aspn_aug_cat2']
            for bidx, aspn_list in enumerate(batch_aspn_aug):
                if py_batch['dial_id'][bidx] not in self.dev_files and py_batch['dial_id'][bidx] not in self.test_files:
                    random.shuffle(aspn_list)
                acts = aspn_list[:cfg.multi_act_sampling_num]
                act_cat = []
                for aspn in acts:
                    act_cat += aspn
                act_cat += self.vocab.sentence_encode(['<eos_a>'])
                py_list.append(act_cat)
            trunc_method = 'pre'
            inputs['aspn_aug_cat' + '_np'] = utils.padSeqs(py_list, truncated=cfg.truncated, trunc_method=trunc_method)
            inputs['aspn_aug_cat' + '_unk_np'] = deepcopy(inputs['aspn_aug_cat'+'_np'])
